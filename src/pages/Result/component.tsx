...
import WrappedMainComponent from 'pages/Welcome';
import TextField from '@material-ui/core/TextField';

import Fade from '@material-ui/core/Fade';

import { validateEmailField, validateTextField } from 'utils/validate';
...
const resultConfig = {
  less30 : {
    ...
    text : "Description/ On the other hand, we denounce with righteous indignation and dislike men who are so beguharms of 1",
    color : "red",
  },
  less60 : {
    header : "Your result is normal",
    text : "Description/ On the other hand, we denounce with righteous indignation and dislike men who are so beguharms of 2",
    color : "yellow",
  },
  more60 : {
...
const Result = (props : any) => {
  ...
  function validateForm():boolean {
    ...
    const getFeedbackError: string = getFeedbackErrors.length ? getFeedbackErrors[0] : '';
  ...
  function validateModalForm():boolean {
    ...
    const getEmailError: string = getEmailErrors.length ? getEmailErrors[0] : '';
    setEmailError(getEmailError);

    const getMessageErrors: string[] = validateTextField(message);
    const getMessageError: string = getMessageErrors.length ? getMessageErrors[0] : '';
    setMessageError(getMessageError);

    return getNameErrors.length === 0 || getEmailErrors.length === 0 || getMessageErrors.length === 0;
  }
  ...
  const [resultNumber, updateResultNumber] = useState(0);
  ...
  const do_circle = () => {
    ...
    props.answers.map((a : any, key: number)=>{
      if(Array.isArray(a.value)){
        ...
        if(a.value==questions[key].answer.value) resultCount = resultCount + 1;
      }
    });
    ...
    updateResultNumber(resultCount);
    ...
    <>
      <Container>
        <section className={classes.root}>
          <Typography className={classes.resultHeader} variant="h3">Thank you for passing this test!</Typography>
          <div className={classes.resultContainer}>

            <div className={classes.cont} data-pct={circlePct}>
              <svg className={classes.svgb} width="200" height="200" viewBox="0 0 200 200" version="1.1" xmlns="http://www.w3.org/2000/svg">
                <circle className={classes.circleBg} r="90" cx="100" cy="100" fill="transparent" strokeDasharray="565.48" strokeDashoffset="0"></circle>
                <circle className={classes.bar} stroke={resultObj.color} r="90" cx="100" cy="100" fill="transparent" strokeDasharray="565.48" strokeDashoffset={strokeDashoffset}></circle>
              </svg>
            </div>
            <div className={classes.resultText}>
              <Typography className={classes.resultTextHeader} variant="h3">{resultObj.header}</Typography>
          ...
          <div className={classes.techContainer}>
              <div className={classes.techLeft}>
                <Typography className={classes.techLeftHeader} variant="h4">Technologies used</Typography>
                <Link href={process.env.REACT_APP_LINK_REP} className={classes.link} underline="none" color="primary">VIEW CODE</Link>
              </div>
            ...
            <div className={classes.footerRight}>
              ...
              >
                 <Fade in={open}>
                    <div className={classes.paper}>
                      <button onClick={() => handleClose()} className={classes.close}><svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M26.0217 9.05103L24.8904 7.91966L16.9707 15.8393L9.05103 7.91968L7.91966 9.05105L15.8393 16.9707L7.9198 24.8902L9.05117 26.0216L16.9707 18.1021L24.8902 26.0216L26.0216 24.8902L18.1021 16.9707L26.0217 9.05103Z" fill="#C4C4C4"/></svg></button>
              ...
              >
                 <Fade in={openThanks}>
                    <div className={classes.paper}>
                      <button onClick={() => handleClose()} className={classes.close}><svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M26.0217 9.05103L24.8904 7.91966L16.9707 15.8393L9.05103 7.91968L7.91966 9.05105L15.8393 16.9707L7.9198 24.8902L9.05117 26.0216L16.9707 18.1021L24.8902 26.0216L26.0216 24.8902L18.1021 16.9707L26.0217 9.05103Z" fill="#C4C4C4"/></svg></button>
                      <Typography className={classes.thanksHeader}>Thank your for you feedback!</Typography>
                    </div>
                  </Fade>
              </Modal>
            </div>
...
const WrappedResultComponent = connect(mapStateToProps2, dispatchActionsToProps2)(Result);
